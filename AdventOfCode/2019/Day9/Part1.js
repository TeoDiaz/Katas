// Thanks https://www.youtube.com/watch?v=2U2OuR_UJVg&list=PL4_jvUW2qVuFKDBX2ZkrvRgW3fNJIQK_A&index=9

function program(codes, inputs) {
  const extraMemory = Array(2 ** 16).fill(0);
  const str = codes
    .split(`,`)
    .concat(extraMemory)
    .map((n) => BigInt(n));
  ...
  for (i = 0; i < str.length && !stop; ) {
    ...
    const code = Number(instructions[0] % 100n);

    const modes = instructions[0].toString().padStart(5, "0");

    const setParam = (pos, value) => {
      const positionMode = modes[3 - pos] == "0";
      const relativeMode = modes[3 - pos] == "2";

      if (positionMode) {
        str[Number(instructions[pos])] = value;
      } else if (relativeMode) {
        str[Number(instructions[pos]) + Number(relativeBase)] = value;
      } else {
        console.log("ERROR!! setting param in direct mode");
      }
    };
    ...
    const getParam = (pos) => {
      ...
      const relativeMode = modes[3 - pos] == "2";
      if (positionMode) {
        return str[Number(instructions[pos])];
      } else if (relativeMode) {
        return str[Number(instructions[pos]) + Number(relativeBase)];
      } else {
    ...
    switch (code) {
      ...
      case 2:
        ...
        break;
      case 3:
        setParam(1, BigInt(inputs.shift()));
        i += 2;
        break;
      case 4:
      ...
      case 99:
        ...
        break;
      default:
        console.log("ERROR!", i, ":", instructions[0]);
        stop = true;
        break;
    }
  }
