...
const debug = (variable) => console.log(JSON.stringify(variable, null, 2));
...
const solve = (input) => {
  ...
  wires[0].map((segment1) => {
    ...
    wires[1].map((segment2) => {
      // intersections check
      // check if segments are one vertical and one horizontal
      if (
        (segment1.from.x == segment1.to.x) ^
        (segment2.from.x == segment2.to.x)
      ) {
        const vertical = segment1.from.x == segment1.to.x ? segment1 : segment2;
        const horizontal =
          segment1.from.x == segment1.to.x ? segment2 : segment1;
        ...
        if (
          ...
          intersection.steps +=
            manhattan(segment1.from, intersection) +
            manhattan(segment2.from, intersection);
